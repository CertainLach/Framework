...
function initServer(rocket, { compiledClientDir, compiledServerDir }) {
    ...
    return async function (ctx) {
        ...
        if (cachedClientStats === null || process.env.NODE_ENV === 'development') {
            cachedClientStats = JSON.parse((await fs_1.readFile(`${compiledClientDir}/stats.json`)).toString());
            cachedServerStats = JSON.parse((await fs_1.readFile(`${compiledServerDir}/stats.json`)).toString());
        }
        const { params, stream, query } = ctx;
        let files = cachedClientStats.assetsByChunkName.main;
        ...
        if (stream.canPushStream) {
            await index_1.asyncEach([...chunkList, ...neededEntryPointScripts], async (file) => {
                ...
                ts.sendFile(path_1.join(compiledClientDir, file));
            });
        }
...
//# sourceMappingURL=initServer.jsx.map