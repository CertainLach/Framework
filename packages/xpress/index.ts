...
export default class XPress<S> extends URouter<XPressRouterContext, S, 'GET' | 'POST' | 'PUT' | 'DELETE' | 'HEAD'> {
     ...
     * @param res
     */
    private populateReqRes(req: IncomingMessage & IIncomingMessageExtension | null, res: ServerResponse & IServerResponseExtension | null) {
        if (req !== null) {
        ...
        if (res !== null) {
            ...
            res.send = (body: string | Buffer | any) => {
                if (res.sent || res.headersSent) throw new Error('Data is already sent!');
                res.sent = true;
    ...
    private async wsHandler(socket: WebSocket) {
        ...
        const next = (e?: any) => {
            if (e) {
                ...
                // If no handlers are set - close socket with 404
                // internalOnMessage/internalOnClose is undocummented fields
                if ((socket as any).internalOnMessage === (socket as any).internalOnClose) {
